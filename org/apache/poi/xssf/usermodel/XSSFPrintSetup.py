def setPaperSize():
    '''returns None\n\n
    setPaperSize(final short size)\n
    setPaperSize(final PaperSize size)\n
    '''
def setScale():
    '''returns None\n\n
    setScale(final short scale)\n
    '''
def setPageStart():
    '''returns None\n\n
    setPageStart(final short start)\n
    '''
def setFitWidth():
    '''returns None\n\n
    setFitWidth(final short width)\n
    '''
def setFitHeight():
    '''returns None\n\n
    setFitHeight(final short height)\n
    '''
def setLeftToRight():
    '''returns None\n\n
    setLeftToRight(final boolean ltor)\n
    '''
def setLandscape():
    '''returns None\n\n
    setLandscape(final boolean ls)\n
    '''
def setValidSettings():
    '''returns None\n\n
    setValidSettings(final boolean valid)\n
    '''
def setNoColor():
    '''returns None\n\n
    setNoColor(final boolean mono)\n
    '''
def setDraft():
    '''returns None\n\n
    setDraft(final boolean d)\n
    '''
def setNotes():
    '''returns None\n\n
    setNotes(final boolean printnotes)\n
    '''
def setNoOrientation():
    '''returns None\n\n
    setNoOrientation(final boolean orientation)\n
    '''
def setUsePage():
    '''returns None\n\n
    setUsePage(final boolean page)\n
    '''
def setHResolution():
    '''returns None\n\n
    setHResolution(final short resolution)\n
    '''
def setVResolution():
    '''returns None\n\n
    setVResolution(final short resolution)\n
    '''
def setHeaderMargin():
    '''returns None\n\n
    setHeaderMargin(final double headermargin)\n
    '''
def setFooterMargin():
    '''returns None\n\n
    setFooterMargin(final double footermargin)\n
    '''
def setCopies():
    '''returns None\n\n
    setCopies(final short copies)\n
    '''
def setOrientation():
    '''returns None\n\n
    setOrientation(final PrintOrientation orientation)\n
    '''
def getOrientation():
    '''returns PrintOrientation\n\n
    getOrientation()\n
    '''
def getCellComment():
    '''returns PrintCellComments\n\n
    getCellComment()\n
    '''
def setPageOrder():
    '''returns None\n\n
    setPageOrder(final PageOrder pageOrder)\n
    '''
def getPageOrder():
    '''returns PageOrder\n\n
    getPageOrder()\n
    '''
def getPaperSize():
    '''returns short\n\n
    getPaperSize()\n
    '''
def getPaperSizeEnum():
    '''returns PaperSize\n\n
    getPaperSizeEnum()\n
    '''
def getScale():
    '''returns short\n\n
    getScale()\n
    '''
def getPageStart():
    '''returns short\n\n
    getPageStart()\n
    '''
def getFitWidth():
    '''returns short\n\n
    getFitWidth()\n
    '''
def getFitHeight():
    '''returns short\n\n
    getFitHeight()\n
    '''
def getLeftToRight():
    '''returns boolean\n\n
    getLeftToRight()\n
    '''
def getLandscape():
    '''returns boolean\n\n
    getLandscape()\n
    '''
def getValidSettings():
    '''returns boolean\n\n
    getValidSettings()\n
    '''
def getNoColor():
    '''returns boolean\n\n
    getNoColor()\n
    '''
def getDraft():
    '''returns boolean\n\n
    getDraft()\n
    '''
def getNotes():
    '''returns boolean\n\n
    getNotes()\n
    '''
def getNoOrientation():
    '''returns boolean\n\n
    getNoOrientation()\n
    '''
def getUsePage():
    '''returns boolean\n\n
    getUsePage()\n
    '''
def getHResolution():
    '''returns short\n\n
    getHResolution()\n
    '''
def getVResolution():
    '''returns short\n\n
    getVResolution()\n
    '''
def getHeaderMargin():
    '''returns double\n\n
    getHeaderMargin()\n
    '''
def getFooterMargin():
    '''returns double\n\n
    getFooterMargin()\n
    '''
def getCopies():
    '''returns short\n\n
    getCopies()\n
    '''
