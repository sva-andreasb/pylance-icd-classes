def size():
    '''returns int\n\n
    size()\n
    '''
def isEmpty():
    '''returns boolean\n\n
    isEmpty()\n
    '''
def containsKey():
    '''returns boolean\n\n
    containsKey(final Object key)\n
    '''
def containsValue():
    '''returns boolean\n\n
    containsValue(final Object value)\n
    '''
def get():
    '''returns V\n\n
    get(final Object key)\n
    '''
def put():
    '''returns V\n\n
    put(final K key, final V value)\n
    '''
def remove():
    '''returns None\n\n
    remove(final Object key)\n
    remove()\n
    '''
def clear():
    '''returns None\n\n
    clear()\n
    '''
def keySet():
    '''returns Set<K>\n\n
    keySet()\n
    '''
def values():
    '''returns Collection<V>\n\n
    values()\n
    '''
def toArray():
    '''returns Object[]\n\n
    toArray()\n
    toArray()\n
    toArray()\n
    '''
def ():
    '''returns WeakRef\n\n
    (final AbstractReferenceMap<K, V> parent, final HashEntry<K, V> next, final int hashCode, final K key, final V value)\n
    (final AbstractReferenceMap<K, V> parent)\n
    (final AbstractReferenceMap<K, V> parent)\n
    (final int hash, final T r, final ReferenceQueue<? super T> q)\n
    (final int hash, final T r, final ReferenceQueue<? super T> q)\n
    '''
def getKey():
    '''returns K\n\n
    getKey()\n
    getKey()\n
    '''
def getValue():
    '''returns V\n\n
    getValue()\n
    getValue()\n
    '''
def setValue():
    '''returns V\n\n
    setValue(final V obj)\n
    setValue(final V value)\n
    '''
def equals():
    '''returns boolean\n\n
    equals(final Object obj)\n
    '''
def hashCode():
    '''returns int\n\n
    hashCode()\n
    hashCode()\n
    hashCode()\n
    '''
def hasNext():
    '''returns boolean\n\n
    hasNext()\n
    '''
def next():
    '''returns K\n\n
    next()\n
    next()\n
    next()\n
    '''
