def NAMESPACE():
    '''returns String\n\n
    NAMESPACE()\n
    '''
def CLASS():
    '''returns Resource\n\n
    CLASS()\n
    '''
def RESTRICTION():
    '''returns Resource\n\n
    RESTRICTION()\n
    '''
def THING():
    '''returns Resource\n\n
    THING()\n
    '''
def NOTHING():
    '''returns Resource\n\n
    NOTHING()\n
    '''
def PROPERTY():
    '''returns Resource\n\n
    PROPERTY()\n
    '''
def OBJECT_PROPERTY():
    '''returns Resource\n\n
    OBJECT_PROPERTY()\n
    '''
def DATATYPE_PROPERTY():
    '''returns Resource\n\n
    DATATYPE_PROPERTY()\n
    '''
def TRANSITIVE_PROPERTY():
    '''returns Resource\n\n
    TRANSITIVE_PROPERTY()\n
    '''
def SYMMETRIC_PROPERTY():
    '''returns Resource\n\n
    SYMMETRIC_PROPERTY()\n
    '''
def FUNCTIONAL_PROPERTY():
    '''returns Resource\n\n
    FUNCTIONAL_PROPERTY()\n
    '''
def INVERSE_FUNCTIONAL_PROPERTY():
    '''returns Resource\n\n
    INVERSE_FUNCTIONAL_PROPERTY()\n
    '''
def ALL_DIFFERENT():
    '''returns Resource\n\n
    ALL_DIFFERENT()\n
    '''
def ONTOLOGY():
    '''returns Resource\n\n
    ONTOLOGY()\n
    '''
def DEPRECATED_CLASS():
    '''returns Resource\n\n
    DEPRECATED_CLASS()\n
    '''
def DEPRECATED_PROPERTY():
    '''returns Resource\n\n
    DEPRECATED_PROPERTY()\n
    '''
def ANNOTATION_PROPERTY():
    '''returns Resource\n\n
    ANNOTATION_PROPERTY()\n
    '''
def ONTOLOGY_PROPERTY():
    '''returns Resource\n\n
    ONTOLOGY_PROPERTY()\n
    '''
def LIST():
    '''returns Resource\n\n
    LIST()\n
    '''
def NIL():
    '''returns Resource\n\n
    NIL()\n
    '''
def DATARANGE():
    '''returns Resource\n\n
    DATARANGE()\n
    '''
def EQUIVALENT_PROPERTY():
    '''returns Property\n\n
    EQUIVALENT_PROPERTY()\n
    '''
def EQUIVALENT_CLASS():
    '''returns Property\n\n
    EQUIVALENT_CLASS()\n
    '''
def DISJOINT_WITH():
    '''returns Property\n\n
    DISJOINT_WITH()\n
    '''
def SAME_INDIVIDUAL_AS():
    '''returns Property\n\n
    SAME_INDIVIDUAL_AS()\n
    '''
def SAME_AS():
    '''returns Property\n\n
    SAME_AS()\n
    '''
def DIFFERENT_FROM():
    '''returns Property\n\n
    DIFFERENT_FROM()\n
    '''
def DISTINCT_MEMBERS():
    '''returns Property\n\n
    DISTINCT_MEMBERS()\n
    '''
def UNION_OF():
    '''returns Property\n\n
    UNION_OF()\n
    '''
def INTERSECTION_OF():
    '''returns Property\n\n
    INTERSECTION_OF()\n
    '''
def COMPLEMENT_OF():
    '''returns Property\n\n
    COMPLEMENT_OF()\n
    '''
def ONE_OF():
    '''returns Property\n\n
    ONE_OF()\n
    '''
def ON_PROPERTY():
    '''returns Property\n\n
    ON_PROPERTY()\n
    '''
def ALL_VALUES_FROM():
    '''returns Property\n\n
    ALL_VALUES_FROM()\n
    '''
def HAS_VALUE():
    '''returns Property\n\n
    HAS_VALUE()\n
    '''
def SOME_VALUES_FROM():
    '''returns Property\n\n
    SOME_VALUES_FROM()\n
    '''
def MIN_CARDINALITY():
    '''returns Property\n\n
    MIN_CARDINALITY()\n
    '''
def MAX_CARDINALITY():
    '''returns Property\n\n
    MAX_CARDINALITY()\n
    '''
def CARDINALITY():
    '''returns Property\n\n
    CARDINALITY()\n
    '''
def INVERSE_OF():
    '''returns Property\n\n
    INVERSE_OF()\n
    '''
def IMPORTS():
    '''returns Property\n\n
    IMPORTS()\n
    '''
def PRIOR_VERSION():
    '''returns Property\n\n
    PRIOR_VERSION()\n
    '''
def BACKWARD_COMPATIBLE_WITH():
    '''returns Property\n\n
    BACKWARD_COMPATIBLE_WITH()\n
    '''
def INCOMPATIBLE_WITH():
    '''returns Property\n\n
    INCOMPATIBLE_WITH()\n
    '''
def SUB_PROPERTY_OF():
    '''returns Property\n\n
    SUB_PROPERTY_OF()\n
    '''
def SUB_CLASS_OF():
    '''returns Property\n\n
    SUB_CLASS_OF()\n
    '''
def DOMAIN():
    '''returns Property\n\n
    DOMAIN()\n
    '''
def RANGE():
    '''returns Property\n\n
    RANGE()\n
    '''
def FIRST():
    '''returns Property\n\n
    FIRST()\n
    '''
def REST():
    '''returns Property\n\n
    REST()\n
    '''
def MIN_CARDINALITY_Q():
    '''returns Property\n\n
    MIN_CARDINALITY_Q()\n
    '''
def MAX_CARDINALITY_Q():
    '''returns Property\n\n
    MAX_CARDINALITY_Q()\n
    '''
def CARDINALITY_Q():
    '''returns Property\n\n
    CARDINALITY_Q()\n
    '''
def HAS_CLASS_Q():
    '''returns Property\n\n
    HAS_CLASS_Q()\n
    '''
def VERSION_INFO():
    '''returns Property\n\n
    VERSION_INFO()\n
    '''
def LABEL():
    '''returns Property\n\n
    LABEL()\n
    '''
def COMMENT():
    '''returns Property\n\n
    COMMENT()\n
    '''
def SEE_ALSO():
    '''returns Property\n\n
    SEE_ALSO()\n
    '''
def IS_DEFINED_BY():
    '''returns Property\n\n
    IS_DEFINED_BY()\n
    '''
def getAxiomTypes():
    '''returns Iterator<Resource>\n\n
    getAxiomTypes()\n
    '''
def getAnnotationProperties():
    '''returns Iterator<Resource>\n\n
    getAnnotationProperties()\n
    '''
def getClassDescriptionTypes():
    '''returns Iterator<Resource>\n\n
    getClassDescriptionTypes()\n
    '''
def getLabel():
    '''returns String\n\n
    getLabel()\n
    '''
def doCheck():
    '''returns boolean\n\n
    doCheck(final Node n, final EnhGraph g)\n
    doCheck(final Node n, final EnhGraph g)\n
    doCheck(final Node n, final EnhGraph eg)\n
    doCheck(final Node n, final EnhGraph g)\n
    doCheck(final Node n, final EnhGraph g)\n
    doCheck(final Node n, final EnhGraph g)\n
    doCheck(final Node n, final EnhGraph g)\n
    doCheck(final Node n, final EnhGraph g)\n
    doCheck(final Node n, final EnhGraph g)\n
    doCheck(final Node n, final EnhGraph g)\n
    doCheck(final Node n, final EnhGraph g)\n
    doCheck(final Node n, final EnhGraph g)\n
    doCheck(final Node n, final EnhGraph g)\n
    doCheck(final Node n, final EnhGraph g)\n
    doCheck(final Node n, final EnhGraph g)\n
    doCheck(final Node n, final EnhGraph g)\n
    doCheck(final Node n, final EnhGraph g)\n
    doCheck(final Node n, final EnhGraph g)\n
    doCheck(final Node n, final EnhGraph g)\n
    doCheck(final Node n, final EnhGraph g)\n
    doCheck(final Node n, final EnhGraph g)\n
    doCheck(final Node n, final EnhGraph g)\n
    '''
def allTypes():
    '''returns Set<Node>\n\n
    allTypes(final Node n, final Graph g)\n
    '''
def intersect():
    '''returns boolean\n\n
    intersect(final Set<Node> nodes, final Resource[] ref)\n
    '''
def hasType():
    '''returns boolean\n\n
    hasType(final Node n, final EnhGraph eg, final Resource[] ref)\n
    '''
def isInferenceGraph():
    '''returns boolean\n\n
    isInferenceGraph(final Graph g)\n
    '''
