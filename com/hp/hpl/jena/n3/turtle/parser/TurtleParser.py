def parse():
    '''    public final void parse()
    '''
def Statement():
    '''    public final void Statement()
    '''
def Directive():
    '''    public final void Directive()
    '''
def TriplesSameSubject():
    '''    public final void TriplesSameSubject()
    '''
def PropertyList():
    '''    public final void PropertyList(final Node s)
    '''
def PropertyListNotEmpty():
    '''    public final void PropertyListNotEmpty(final Node s)
    '''
def ObjectList():
    '''    public final void ObjectList(final Node s, final Node p)
    '''
def Object():
    '''    public final void Object(final Node s, final Node p)
    '''
def Verb():
    '''    public final Node Verb()
    '''
def TriplesNode():
    '''    public final Node TriplesNode()
    '''
def BlankNodePropertyList():
    '''    public final Node BlankNodePropertyList()
    '''
def Collection():
    '''    public final Node Collection()
    '''
def GraphNode():
    '''    public final Node GraphNode()
    '''
def VarOrTerm():
    '''    public final Node VarOrTerm()
    '''
def Formula():
    '''    public final Node Formula()
    '''
def Var():
    '''    public final Node Var()
    '''
def GraphTerm():
    '''    public final Node GraphTerm()
    '''
def NumericLiteral():
    '''    public final Node NumericLiteral()
    '''
def RDFLiteral():
    '''    public final Node RDFLiteral()
    '''
def Langtag():
    '''    public final String Langtag()
    '''
def AnyDirective():
    '''    public final Token AnyDirective()
    '''
def BooleanLiteral():
    '''    public final Node BooleanLiteral()
    '''
def String():
    '''    public final String String()
    '''
def IRIref():
    '''    public final String IRIref()
    '''
def PrefixedName():
    '''    public final String PrefixedName()
    '''
def BlankNode():
    '''    public final Node BlankNode()
    '''
def IRI_REF():
    '''    public final String IRI_REF()
    '''
def TurtleParser():
    '''    public TurtleParser(final InputStream stream)
    public TurtleParser(final InputStream stream, final String encoding)
    public TurtleParser(final Reader stream)
    public TurtleParser(final TurtleParserTokenManager tm)
    '''
def ReInit():
    '''    public void ReInit(final InputStream stream)
    public void ReInit(final InputStream stream, final String encoding)
    public void ReInit(final Reader stream)
    public void ReInit(final TurtleParserTokenManager tm)
    '''
def getNextToken():
    '''    public final Token getNextToken()
    '''
def getToken():
    '''    public final Token getToken(final int index)
    '''
def generateParseException():
    '''    public ParseException generateParseException()
    '''
def enable_tracing():
    '''    public final void enable_tracing()
    '''
def disable_tracing():
    '''    public final void disable_tracing()
    '''
