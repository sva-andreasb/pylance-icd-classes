def getClassName():
    '''returns String\n\n
    getClassName()\n
    '''
def ():
    '''returns DiscoveryRunJdo\n\n
    (final Guid guid, final TopologyActionContext ctx)\n
    ()\n
    '''
def getVersion():
    '''returns long\n\n
    getVersion()\n
    '''
def setVersion():
    '''returns None\n\n
    setVersion(final long version)\n
    '''
def getPk():
    '''returns String\n\n
    getPk()\n
    '''
def setPk():
    '''returns None\n\n
    setPk(final String pk)\n
    '''
def getDeleted():
    '''returns boolean\n\n
    getDeleted()\n
    '''
def setDeleted():
    '''returns None\n\n
    setDeleted(final boolean d)\n
    '''
def delete():
    '''returns None\n\n
    delete()\n
    delete(final Set removedGuidSet)\n
    '''
def undelete():
    '''returns None\n\n
    undelete()\n
    '''
def getRunId():
    '''returns long\n\n
    getRunId()\n
    '''
def setRunId():
    '''returns None\n\n
    setRunId(final long id)\n
    '''
def getPriority():
    '''returns int\n\n
    getPriority()\n
    '''
def setPriority():
    '''returns None\n\n
    setPriority(final int priority)\n
    '''
def getCreatedBy():
    '''returns String\n\n
    getCreatedBy()\n
    '''
def getLastModifiedTime():
    '''returns long\n\n
    getLastModifiedTime()\n
    '''
def getLastModifiedBy():
    '''returns String\n\n
    getLastModifiedBy()\n
    '''
def setCreatedBy():
    '''returns None\n\n
    setCreatedBy(final String s)\n
    '''
def setLastModifiedTime():
    '''returns None\n\n
    setLastModifiedTime(final long l)\n
    '''
def setLastModifiedBy():
    '''returns None\n\n
    setLastModifiedBy(final String s)\n
    '''
def getDisplayName():
    '''returns String\n\n
    getDisplayName()\n
    '''
def setDisplayName():
    '''returns None\n\n
    setDisplayName(final String a)\n
    '''
def hasDisplayName():
    '''returns boolean\n\n
    hasDisplayName()\n
    '''
def setAttrPrios():
    '''returns None\n\n
    setAttrPrios(final String attrPrios)\n
    '''
def getAttrPrios():
    '''returns String\n\n
    getAttrPrios()\n
    '''
def hasAttrPrios():
    '''returns boolean\n\n
    hasAttrPrios()\n
    '''
def getDiscoverRunId():
    '''returns long\n\n
    getDiscoverRunId()\n
    '''
def setDiscoverRunId():
    '''returns None\n\n
    setDiscoverRunId(final long a)\n
    '''
def hasDiscoverRunId():
    '''returns boolean\n\n
    hasDiscoverRunId()\n
    '''
def getDiscoveryProfile():
    '''returns DiscoveryProfile\n\n
    getDiscoveryProfile()\n
    '''
def setDiscoveryProfile():
    '''returns None\n\n
    setDiscoveryProfile(final DiscoveryProfile a)\n
    '''
def hasDiscoveryProfile():
    '''returns boolean\n\n
    hasDiscoveryProfile()\n
    '''
def getInitiator():
    '''returns String\n\n
    getInitiator()\n
    '''
def setInitiator():
    '''returns None\n\n
    setInitiator(final String a)\n
    '''
def hasInitiator():
    '''returns boolean\n\n
    hasInitiator()\n
    '''
def getScheduled():
    '''returns boolean\n\n
    getScheduled()\n
    '''
def setScheduled():
    '''returns None\n\n
    setScheduled(final boolean a)\n
    '''
def hasScheduled():
    '''returns boolean\n\n
    hasScheduled()\n
    '''
def getStartTime():
    '''returns long\n\n
    getStartTime()\n
    '''
def setStartTime():
    '''returns None\n\n
    setStartTime(final long a)\n
    '''
def hasStartTime():
    '''returns boolean\n\n
    hasStartTime()\n
    '''
def getEndTime():
    '''returns long\n\n
    getEndTime()\n
    '''
def setEndTime():
    '''returns None\n\n
    setEndTime(final long a)\n
    '''
def hasEndTime():
    '''returns boolean\n\n
    hasEndTime()\n
    '''
def getScopeElements():
    '''returns ScopeElement[]\n\n
    getScopeElements()\n
    '''
def setScopeElements():
    '''returns None\n\n
    setScopeElements(final ScopeElement[] a)\n
    '''
def hasScopeElements():
    '''returns boolean\n\n
    hasScopeElements()\n
    '''
def addScopeElements():
    '''returns boolean\n\n
    addScopeElements(final ScopeElement obj)\n
    '''
def removeScopeElements():
    '''returns boolean\n\n
    removeScopeElements(final ScopeElement obj)\n
    '''
def getTerminationType():
    '''returns int\n\n
    getTerminationType()\n
    '''
def setTerminationType():
    '''returns None\n\n
    setTerminationType(final int a)\n
    '''
def hasTerminationType():
    '''returns boolean\n\n
    hasTerminationType()\n
    '''
def getStatus():
    '''returns int\n\n
    getStatus()\n
    '''
def setStatus():
    '''returns None\n\n
    setStatus(final int a)\n
    '''
def hasStatus():
    '''returns boolean\n\n
    hasStatus()\n
    '''
def getRunType():
    '''returns int\n\n
    getRunType()\n
    '''
def setRunType():
    '''returns None\n\n
    setRunType(final int a)\n
    '''
def hasRunType():
    '''returns boolean\n\n
    hasRunType()\n
    '''
def getCause():
    '''returns String\n\n
    getCause()\n
    '''
def setCause():
    '''returns None\n\n
    setCause(final String a)\n
    '''
def hasCause():
    '''returns boolean\n\n
    hasCause()\n
    '''
def getDiscoveryProgressLog():
    '''returns String\n\n
    getDiscoveryProgressLog()\n
    '''
def setDiscoveryProgressLog():
    '''returns None\n\n
    setDiscoveryProgressLog(final String a)\n
    '''
def hasDiscoveryProgressLog():
    '''returns boolean\n\n
    hasDiscoveryProgressLog()\n
    '''
def getRunName():
    '''returns String\n\n
    getRunName()\n
    '''
def setRunName():
    '''returns None\n\n
    setRunName(final String a)\n
    '''
def hasRunName():
    '''returns boolean\n\n
    hasRunName()\n
    '''
def getRoles():
    '''returns Role[]\n\n
    getRoles()\n
    '''
def setRoles():
    '''returns None\n\n
    setRoles(final Role[] a)\n
    '''
def hasRoles():
    '''returns boolean\n\n
    hasRoles()\n
    '''
def addRoles():
    '''returns boolean\n\n
    addRoles(final Role obj)\n
    '''
def removeRoles():
    '''returns boolean\n\n
    removeRoles(final Role obj)\n
    '''
def getManagedSystemName():
    '''returns String\n\n
    getManagedSystemName()\n
    '''
def setManagedSystemName():
    '''returns None\n\n
    setManagedSystemName(final String a)\n
    '''
def hasManagedSystemName():
    '''returns boolean\n\n
    hasManagedSystemName()\n
    '''
def getObjectType():
    '''returns String\n\n
    getObjectType()\n
    '''
def setObjectType():
    '''returns None\n\n
    setObjectType(final String a)\n
    '''
def hasObjectType():
    '''returns boolean\n\n
    hasObjectType()\n
    '''
def getExtendedAttributes():
    '''returns byte[]\n\n
    getExtendedAttributes()\n
    '''
def setExtendedAttributes():
    '''returns None\n\n
    setExtendedAttributes(final byte[] a)\n
    '''
def hasExtendedAttributes():
    '''returns boolean\n\n
    hasExtendedAttributes()\n
    '''
def getBidiFlag():
    '''returns int\n\n
    getBidiFlag()\n
    '''
def setBidiFlag():
    '''returns None\n\n
    setBidiFlag(final int a)\n
    '''
def hasBidiFlag():
    '''returns boolean\n\n
    hasBidiFlag()\n
    '''
def getDescription():
    '''returns String\n\n
    getDescription()\n
    '''
def setDescription():
    '''returns None\n\n
    setDescription(final String a)\n
    '''
def hasDescription():
    '''returns boolean\n\n
    hasDescription()\n
    '''
def getCmdbSource():
    '''returns Guid\n\n
    getCmdbSource()\n
    '''
def setCmdbSource():
    '''returns None\n\n
    setCmdbSource(final Guid guid)\n
    '''
def hasCmdbSource():
    '''returns boolean\n\n
    hasCmdbSource()\n
    '''
def getGuid():
    '''returns Guid\n\n
    getGuid()\n
    '''
def setGuid():
    '''returns None\n\n
    setGuid(final Guid guid)\n
    '''
def hasGuid():
    '''returns boolean\n\n
    hasGuid()\n
    '''
def getCDMSource():
    '''returns String\n\n
    getCDMSource()\n
    '''
def setCDMSource():
    '''returns None\n\n
    setCDMSource(final String a)\n
    '''
def hasCDMSource():
    '''returns boolean\n\n
    hasCDMSource()\n
    '''
def getAdminState():
    '''returns int\n\n
    getAdminState()\n
    '''
def setAdminState():
    '''returns None\n\n
    setAdminState(final int a)\n
    '''
def hasAdminState():
    '''returns boolean\n\n
    hasAdminState()\n
    '''
def getSourceToken():
    '''returns String\n\n
    getSourceToken()\n
    '''
def setSourceToken():
    '''returns None\n\n
    setSourceToken(final String a)\n
    '''
def hasSourceToken():
    '''returns boolean\n\n
    hasSourceToken()\n
    '''
def getContextIp():
    '''returns String\n\n
    getContextIp()\n
    '''
def setContextIp():
    '''returns None\n\n
    setContextIp(final String a)\n
    '''
def hasContextIp():
    '''returns boolean\n\n
    hasContextIp()\n
    '''
def getLabel():
    '''returns String\n\n
    getLabel()\n
    '''
def setLabel():
    '''returns None\n\n
    setLabel(final String a)\n
    '''
def hasLabel():
    '''returns boolean\n\n
    hasLabel()\n
    '''
def getBidiFormat():
    '''returns String\n\n
    getBidiFormat()\n
    '''
def setBidiFormat():
    '''returns None\n\n
    setBidiFormat(final String a)\n
    '''
def hasBidiFormat():
    '''returns boolean\n\n
    hasBidiFormat()\n
    '''
def getAllAttributes():
    '''returns Map\n\n
    getAllAttributes()\n
    '''
def generateDisplayName():
    '''returns String\n\n
    generateDisplayName()\n
    '''
def sameJdo():
    '''returns boolean\n\n
    sameJdo(final DiscoveryRun obj, final Map objKeys, final TopologyActionContext ctx, final JdoUpdateMap updateMap)\n
    '''
def sameJdoTest():
    '''returns boolean\n\n
    sameJdoTest(final DiscoveryRun obj, final TopologyActionContext ctx, final JdoUpdateMap updateMap)\n
    '''
def similarJdo():
    '''returns boolean\n\n
    similarJdo(final DiscoveryRunJdo obj)\n
    '''
def updateJdoByObj():
    '''returns None\n\n
    updateJdoByObj(final DiscoveryRun obj, final List guidMap, final TopologyActionContext ctx, final Set processedObjs, final JdoUpdateMap updateMap, final Map stknGuidMap, final String attrPriosFromDb)\n
    '''
def copyJdo():
    '''returns None\n\n
    copyJdo(final BaseJdo copyToJdo, final JdoUpdateMap updateMap)\n
    '''
def removeJdoRefs():
    '''returns None\n\n
    removeJdoRefs(final TopologyActionContext ctx)\n
    removeJdoRefs(final TopologyActionContext ctx, final Set removedGuidSet)\n
    '''
def deleteJdoRefs():
    '''returns None\n\n
    deleteJdoRefs(final TopologyActionContext ctx)\n
    deleteJdoRefs(final TopologyActionContext ctx, final Set removedGuidSet)\n
    '''
def restoreJdo():
    '''returns None\n\n
    restoreJdo(final TopologyActionContext ctx)\n
    '''
def compareJdo():
    '''returns ObjectCompareResults\n\n
    compareJdo(final BaseJdo o, final boolean includeDynamic, final boolean includeDeep)\n
    compareJdo(final BaseJdo o, final boolean includeDynamic, final boolean includeDeep, final TwoDimensionalMap refMap)\n
    '''
def compareJdoForMerge():
    '''returns ObjectCompareResults\n\n
    compareJdoForMerge(final BaseJdo o, final boolean includeDynamic, final boolean includeDeep)\n
    compareJdoForMerge(final BaseJdo o, final boolean includeDynamic, final boolean includeDeep, final TwoDimensionalMap refMap)\n
    '''
def versionedJdoExists():
    '''returns boolean\n\n
    versionedJdoExists(final TopologyActionContext ctx)\n
    '''
def jdoNewInstance():
    '''returns PersistenceCapable\n\n
    jdoNewInstance(final StateManager jdoStateManager, final Object o)\n
    jdoNewInstance(final StateManager jdoStateManager)\n
    '''
def jdoReplaceField():
    '''returns None\n\n
    jdoReplaceField(final int n)\n
    '''
def jdoReplaceFields():
    '''returns None\n\n
    jdoReplaceFields(final int[] array)\n
    '''
def jdoProvideField():
    '''returns None\n\n
    jdoProvideField(final int n)\n
    '''
def jdoProvideFields():
    '''returns None\n\n
    jdoProvideFields(final int[] array)\n
    '''
def jdoCopyFields():
    '''returns None\n\n
    jdoCopyFields(final Object o, final int[] array)\n
    '''
def jdoGetPersistenceManager():
    '''returns PersistenceManager\n\n
    jdoGetPersistenceManager()\n
    '''
def jdoGetObjectId():
    '''returns Object\n\n
    jdoGetObjectId()\n
    '''
def jdoGetTransactionalObjectId():
    '''returns Object\n\n
    jdoGetTransactionalObjectId()\n
    '''
def jdoIsDeleted():
    '''returns boolean\n\n
    jdoIsDeleted()\n
    '''
def jdoIsDirty():
    '''returns boolean\n\n
    jdoIsDirty()\n
    '''
def jdoIsNew():
    '''returns boolean\n\n
    jdoIsNew()\n
    '''
def jdoIsPersistent():
    '''returns boolean\n\n
    jdoIsPersistent()\n
    '''
def jdoIsTransactional():
    '''returns boolean\n\n
    jdoIsTransactional()\n
    '''
def jdoPreSerialize():
    '''returns None\n\n
    jdoPreSerialize()\n
    '''
def jdoMakeDirty():
    '''returns None\n\n
    jdoMakeDirty(final String s)\n
    '''
def jdoReplaceFlags():
    '''returns None\n\n
    jdoReplaceFlags()\n
    '''
def jdoCopyKeyFieldsToObjectId():
    '''returns None\n\n
    jdoCopyKeyFieldsToObjectId(final PersistenceCapable$ObjectIdFieldSupplier persistenceCapable$ObjectIdFieldSupplier, final Object o)\n
    jdoCopyKeyFieldsToObjectId(final Object o)\n
    '''
def jdoCopyKeyFieldsFromObjectId():
    '''returns None\n\n
    jdoCopyKeyFieldsFromObjectId(final PersistenceCapable$ObjectIdFieldConsumer persistenceCapable$ObjectIdFieldConsumer, final Object o)\n
    jdoCopyKeyFieldsFromObjectId(final Object o)\n
    '''
def jdoNewObjectIdInstance():
    '''returns Object\n\n
    jdoNewObjectIdInstance(final String pk)\n
    jdoNewObjectIdInstance()\n
    '''
