def ():
    '''returns SnmpTrapForwarderConfigurationImpl\n\n
    ()\n
    (final Map m)\n
    '''
def getTrapDestination():
    '''returns String\n\n
    getTrapDestination()\n
    '''
def hasTrapDestination():
    '''returns boolean\n\n
    hasTrapDestination()\n
    '''
def setTrapDestination():
    '''returns None\n\n
    setTrapDestination(final String destination)\n
    '''
def getTrapPort():
    '''returns int\n\n
    getTrapPort()\n
    '''
def hasTrapPort():
    '''returns boolean\n\n
    hasTrapPort()\n
    '''
def setTrapPort():
    '''returns None\n\n
    setTrapPort(final int port)\n
    '''
def getTrapCommunity():
    '''returns String\n\n
    getTrapCommunity()\n
    '''
def hasTrapCommunity():
    '''returns boolean\n\n
    hasTrapCommunity()\n
    '''
def setTrapCommunity():
    '''returns None\n\n
    setTrapCommunity(final String community)\n
    '''
def getGuid():
    '''returns Guid\n\n
    getGuid()\n
    '''
def hasGuid():
    '''returns boolean\n\n
    hasGuid()\n
    '''
def setGuid():
    '''returns None\n\n
    setGuid(final Guid guid)\n
    '''
def getDisplayName():
    '''returns String\n\n
    getDisplayName()\n
    '''
def hasDisplayName():
    '''returns boolean\n\n
    hasDisplayName()\n
    '''
def getContextIp():
    '''returns String\n\n
    getContextIp()\n
    '''
def hasContextIp():
    '''returns boolean\n\n
    hasContextIp()\n
    '''
def setContextIp():
    '''returns None\n\n
    setContextIp(final String ctx)\n
    '''
def getObjectType():
    '''returns String\n\n
    getObjectType()\n
    '''
def hasObjectType():
    '''returns boolean\n\n
    hasObjectType()\n
    '''
def setObjectType():
    '''returns None\n\n
    setObjectType(final String type)\n
    '''
def getCDMSource():
    '''returns String\n\n
    getCDMSource()\n
    '''
def hasCDMSource():
    '''returns boolean\n\n
    hasCDMSource()\n
    '''
def setCDMSource():
    '''returns None\n\n
    setCDMSource(final String source)\n
    '''
def getSourceToken():
    '''returns String\n\n
    getSourceToken()\n
    '''
def hasSourceToken():
    '''returns boolean\n\n
    hasSourceToken()\n
    '''
def setSourceToken():
    '''returns None\n\n
    setSourceToken(final String sourceToken)\n
    '''
def getCmdbSource():
    '''returns Guid\n\n
    getCmdbSource()\n
    '''
def hasCmdbSource():
    '''returns boolean\n\n
    hasCmdbSource()\n
    '''
def setCmdbSource():
    '''returns None\n\n
    setCmdbSource(final Guid cmdbsource)\n
    '''
def getLabel():
    '''returns String\n\n
    getLabel()\n
    '''
def hasLabel():
    '''returns boolean\n\n
    hasLabel()\n
    '''
def setLabel():
    '''returns None\n\n
    setLabel(final String label)\n
    '''
def getDescription():
    '''returns String\n\n
    getDescription()\n
    '''
def hasDescription():
    '''returns boolean\n\n
    hasDescription()\n
    '''
def setDescription():
    '''returns None\n\n
    setDescription(final String description)\n
    '''
def getAdminState():
    '''returns int\n\n
    getAdminState()\n
    '''
def hasAdminState():
    '''returns boolean\n\n
    hasAdminState()\n
    '''
def setAdminState():
    '''returns None\n\n
    setAdminState(final int state)\n
    '''
def getBidiFlag():
    '''returns int\n\n
    getBidiFlag()\n
    '''
def hasBidiFlag():
    '''returns boolean\n\n
    hasBidiFlag()\n
    '''
def setBidiFlag():
    '''returns None\n\n
    setBidiFlag(final int bidiflag)\n
    '''
def getBidiFormat():
    '''returns String\n\n
    getBidiFormat()\n
    '''
def hasBidiFormat():
    '''returns boolean\n\n
    hasBidiFormat()\n
    '''
def setBidiFormat():
    '''returns None\n\n
    setBidiFormat(final String bidiformat)\n
    '''
def getAllAttributes():
    '''returns Map\n\n
    getAllAttributes()\n
    '''
def getCreatedBy():
    '''returns String\n\n
    getCreatedBy()\n
    '''
def hasCreatedBy():
    '''returns boolean\n\n
    hasCreatedBy()\n
    '''
def getLastModifiedTime():
    '''returns long\n\n
    getLastModifiedTime()\n
    '''
def hasLastModifiedTime():
    '''returns boolean\n\n
    hasLastModifiedTime()\n
    '''
def getLastModifiedBy():
    '''returns String\n\n
    getLastModifiedBy()\n
    '''
def hasLastModifiedBy():
    '''returns boolean\n\n
    hasLastModifiedBy()\n
    '''
def getExtendedAttributes():
    '''returns byte[]\n\n
    getExtendedAttributes()\n
    '''
def hasExtendedAttributes():
    '''returns boolean\n\n
    hasExtendedAttributes()\n
    '''
def setExtendedAttributes():
    '''returns None\n\n
    setExtendedAttributes(final byte[] eAttribMap)\n
    '''
def getComponentType():
    '''returns String\n\n
    getComponentType()\n
    '''
def hasComponentType():
    '''returns boolean\n\n
    hasComponentType()\n
    '''
def setComponentType():
    '''returns None\n\n
    setComponentType(final String componentType)\n
    '''
def getSeverityThreshold():
    '''returns int\n\n
    getSeverityThreshold()\n
    '''
def hasSeverityThreshold():
    '''returns boolean\n\n
    hasSeverityThreshold()\n
    '''
def setSeverityThreshold():
    '''returns None\n\n
    setSeverityThreshold(final int threshold)\n
    '''
def getCategory():
    '''returns int\n\n
    getCategory()\n
    '''
def hasCategory():
    '''returns boolean\n\n
    hasCategory()\n
    '''
def setCategory():
    '''returns None\n\n
    setCategory(final int category)\n
    '''
def getName():
    '''returns String\n\n
    getName()\n
    '''
def hasName():
    '''returns boolean\n\n
    hasName()\n
    '''
def setName():
    '''returns None\n\n
    setName(final String name)\n
    '''
def getFamilyName():
    '''returns String\n\n
    getFamilyName()\n
    '''
def hasFamilyName():
    '''returns boolean\n\n
    hasFamilyName()\n
    '''
def setFamilyName():
    '''returns None\n\n
    setFamilyName(final String name)\n
    '''
def getEnabled():
    '''returns boolean\n\n
    getEnabled()\n
    '''
def hasEnabled():
    '''returns boolean\n\n
    hasEnabled()\n
    '''
def setEnabled():
    '''returns None\n\n
    setEnabled(final boolean enabled)\n
    '''
def getOperationalStatus():
    '''returns int\n\n
    getOperationalStatus()\n
    '''
def hasOperationalStatus():
    '''returns boolean\n\n
    hasOperationalStatus()\n
    '''
def setOperationalStatus():
    '''returns None\n\n
    setOperationalStatus(final int status)\n
    '''
def getRejectionReason():
    '''returns String\n\n
    getRejectionReason()\n
    '''
def hasRejectionReason():
    '''returns boolean\n\n
    hasRejectionReason()\n
    '''
def setRejectionReason():
    '''returns None\n\n
    setRejectionReason(final String reason)\n
    '''
def getMssGuid():
    '''returns String\n\n
    getMssGuid()\n
    '''
def hasMssGuid():
    '''returns boolean\n\n
    hasMssGuid()\n
    '''
def setMssGuid():
    '''returns None\n\n
    setMssGuid(final String mssGuid)\n
    '''
def toString():
    '''returns String\n\n
    toString()\n
    '''
