def ApplicationDeploymentPlanImpl():
'''public ApplicationDeploymentPlanImpl()
public ApplicationDeploymentPlanImpl(final Map m)
'''
pass
def setPreviousDeploymentPlan():
'''public void setPreviousDeploymentPlan(final String s)
'''
pass
def getPreviousDeploymentPlan():
'''public String getPreviousDeploymentPlan()
'''
pass
def hasPreviousDeploymentPlan():
'''public boolean hasPreviousDeploymentPlan()
'''
pass
def setName():
'''public void setName(final String s)
'''
pass
def getName():
'''public String getName()
'''
pass
def hasName():
'''public boolean hasName()
'''
pass
def setStatus():
'''public void setStatus(final int s)
'''
pass
def getStatus():
'''public int getStatus()
'''
pass
def hasStatus():
'''public boolean hasStatus()
'''
pass
def setApplicationDefinitionName():
'''public void setApplicationDefinitionName(final String s)
'''
pass
def getApplicationDefinitionName():
'''public String getApplicationDefinitionName()
'''
pass
def hasApplicationDefinitionName():
'''public boolean hasApplicationDefinitionName()
'''
pass
def setApplicationDefinitionVersion():
'''public void setApplicationDefinitionVersion(final String s)
'''
pass
def getApplicationDefinitionVersion():
'''public String getApplicationDefinitionVersion()
'''
pass
def hasApplicationDefinitionVersion():
'''public boolean hasApplicationDefinitionVersion()
'''
pass
def setWorkflow():
'''public void setWorkflow(final Workflow workflow)
'''
pass
def getWorkflow():
'''public Workflow getWorkflow()
'''
pass
def hasWorkflow():
'''public boolean hasWorkflow()
'''
pass
def setWorkflowDefinition():
'''public void setWorkflowDefinition(final Workflow workflow)
'''
pass
def getWorkflowDefinition():
'''public Workflow getWorkflowDefinition()
'''
pass
def hasWorkflowDefinition():
'''public boolean hasWorkflowDefinition()
'''
pass
def setModuleMappings():
'''public void setModuleMappings(final ModuleMapping[] s)
'''
pass
def getModuleMappings():
'''public ModuleMapping[] getModuleMappings()
'''
pass
def hasModuleMappings():
'''public boolean hasModuleMappings()
'''
pass
def getGuid():
'''public Guid getGuid()
'''
pass
def hasGuid():
'''public boolean hasGuid()
'''
pass
def setGuid():
'''public void setGuid(final Guid guid)
'''
pass
def getDisplayName():
'''public String getDisplayName()
'''
pass
def hasDisplayName():
'''public boolean hasDisplayName()
'''
pass
def getContextIp():
'''public String getContextIp()
'''
pass
def hasContextIp():
'''public boolean hasContextIp()
'''
pass
def setContextIp():
'''public void setContextIp(final String ctx)
'''
pass
def getObjectType():
'''public String getObjectType()
'''
pass
def hasObjectType():
'''public boolean hasObjectType()
'''
pass
def setObjectType():
'''public void setObjectType(final String type)
'''
pass
def getCDMSource():
'''public String getCDMSource()
'''
pass
def hasCDMSource():
'''public boolean hasCDMSource()
'''
pass
def setCDMSource():
'''public void setCDMSource(final String source)
'''
pass
def getSourceToken():
'''public String getSourceToken()
'''
pass
def hasSourceToken():
'''public boolean hasSourceToken()
'''
pass
def setSourceToken():
'''public void setSourceToken(final String sourceToken)
'''
pass
def getCmdbSource():
'''public Guid getCmdbSource()
'''
pass
def hasCmdbSource():
'''public boolean hasCmdbSource()
'''
pass
def setCmdbSource():
'''public void setCmdbSource(final Guid cmdbsource)
'''
pass
def getLabel():
'''public String getLabel()
'''
pass
def hasLabel():
'''public boolean hasLabel()
'''
pass
def setLabel():
'''public void setLabel(final String label)
'''
pass
def getDescription():
'''public String getDescription()
'''
pass
def hasDescription():
'''public boolean hasDescription()
'''
pass
def setDescription():
'''public void setDescription(final String description)
'''
pass
def getAdminState():
'''public int getAdminState()
'''
pass
def hasAdminState():
'''public boolean hasAdminState()
'''
pass
def setAdminState():
'''public void setAdminState(final int state)
'''
pass
def getBidiFlag():
'''public int getBidiFlag()
'''
pass
def hasBidiFlag():
'''public boolean hasBidiFlag()
'''
pass
def setBidiFlag():
'''public void setBidiFlag(final int bidiflag)
'''
pass
def getBidiFormat():
'''public String getBidiFormat()
'''
pass
def hasBidiFormat():
'''public boolean hasBidiFormat()
'''
pass
def setBidiFormat():
'''public void setBidiFormat(final String bidiformat)
'''
pass
def getAllAttributes():
'''public Map getAllAttributes()
'''
pass
def getCreatedBy():
'''public String getCreatedBy()
'''
pass
def hasCreatedBy():
'''public boolean hasCreatedBy()
'''
pass
def getLastModifiedTime():
'''public long getLastModifiedTime()
'''
pass
def hasLastModifiedTime():
'''public boolean hasLastModifiedTime()
'''
pass
def getLastModifiedBy():
'''public String getLastModifiedBy()
'''
pass
def hasLastModifiedBy():
'''public boolean hasLastModifiedBy()
'''
pass
def getExtendedAttributes():
'''public byte[] getExtendedAttributes()
'''
pass
def hasExtendedAttributes():
'''public boolean hasExtendedAttributes()
'''
pass
def setExtendedAttributes():
'''public void setExtendedAttributes(final byte[] eAttribMap)
'''
pass
def toString():
'''public String toString()
'''
pass
