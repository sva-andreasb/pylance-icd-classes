def ():
    '''returns ActionImpl\n\n
    ()\n
    (final Map m)\n
    '''
def setName():
    '''returns None\n\n
    setName(final String s)\n
    '''
def getName():
    '''returns String\n\n
    getName()\n
    '''
def hasName():
    '''returns boolean\n\n
    hasName()\n
    '''
def setType():
    '''returns None\n\n
    setType(final String s)\n
    '''
def getType():
    '''returns String\n\n
    getType()\n
    '''
def hasType():
    '''returns boolean\n\n
    hasType()\n
    '''
def setOwner():
    '''returns None\n\n
    setOwner(final String s)\n
    '''
def getOwner():
    '''returns String\n\n
    getOwner()\n
    '''
def hasOwner():
    '''returns boolean\n\n
    hasOwner()\n
    '''
def setCommandType():
    '''returns None\n\n
    setCommandType(final String s)\n
    '''
def getCommandType():
    '''returns String\n\n
    getCommandType()\n
    '''
def hasCommandType():
    '''returns boolean\n\n
    hasCommandType()\n
    '''
def setCommand():
    '''returns None\n\n
    setCommand(final String s)\n
    '''
def getCommand():
    '''returns String\n\n
    getCommand()\n
    '''
def hasCommand():
    '''returns boolean\n\n
    hasCommand()\n
    '''
def setFileLocation():
    '''returns None\n\n
    setFileLocation(final FileLocation fl)\n
    '''
def getFileLocation():
    '''returns FileLocation\n\n
    getFileLocation()\n
    '''
def hasFileLocation():
    '''returns boolean\n\n
    hasFileLocation()\n
    '''
def setTargetPath():
    '''returns None\n\n
    setTargetPath(final String s)\n
    '''
def getTargetPath():
    '''returns String\n\n
    getTargetPath()\n
    '''
def hasTargetPath():
    '''returns boolean\n\n
    hasTargetPath()\n
    '''
def setUserName():
    '''returns None\n\n
    setUserName(final String s)\n
    '''
def getUserName():
    '''returns String\n\n
    getUserName()\n
    '''
def hasUserName():
    '''returns boolean\n\n
    hasUserName()\n
    '''
def setWorkingDirectory():
    '''returns None\n\n
    setWorkingDirectory(final String s)\n
    '''
def getWorkingDirectory():
    '''returns String\n\n
    getWorkingDirectory()\n
    '''
def hasWorkingDirectory():
    '''returns boolean\n\n
    hasWorkingDirectory()\n
    '''
def setParameters():
    '''returns None\n\n
    setParameters(final String s)\n
    '''
def getParameters():
    '''returns String\n\n
    getParameters()\n
    '''
def hasParameters():
    '''returns boolean\n\n
    hasParameters()\n
    '''
def getGuid():
    '''returns Guid\n\n
    getGuid()\n
    '''
def hasGuid():
    '''returns boolean\n\n
    hasGuid()\n
    '''
def setGuid():
    '''returns None\n\n
    setGuid(final Guid guid)\n
    '''
def getDisplayName():
    '''returns String\n\n
    getDisplayName()\n
    '''
def hasDisplayName():
    '''returns boolean\n\n
    hasDisplayName()\n
    '''
def getContextIp():
    '''returns String\n\n
    getContextIp()\n
    '''
def hasContextIp():
    '''returns boolean\n\n
    hasContextIp()\n
    '''
def setContextIp():
    '''returns None\n\n
    setContextIp(final String ctx)\n
    '''
def getObjectType():
    '''returns String\n\n
    getObjectType()\n
    '''
def hasObjectType():
    '''returns boolean\n\n
    hasObjectType()\n
    '''
def setObjectType():
    '''returns None\n\n
    setObjectType(final String type)\n
    '''
def getCDMSource():
    '''returns String\n\n
    getCDMSource()\n
    '''
def hasCDMSource():
    '''returns boolean\n\n
    hasCDMSource()\n
    '''
def setCDMSource():
    '''returns None\n\n
    setCDMSource(final String source)\n
    '''
def getSourceToken():
    '''returns String\n\n
    getSourceToken()\n
    '''
def hasSourceToken():
    '''returns boolean\n\n
    hasSourceToken()\n
    '''
def setSourceToken():
    '''returns None\n\n
    setSourceToken(final String sourceToken)\n
    '''
def getCmdbSource():
    '''returns Guid\n\n
    getCmdbSource()\n
    '''
def hasCmdbSource():
    '''returns boolean\n\n
    hasCmdbSource()\n
    '''
def setCmdbSource():
    '''returns None\n\n
    setCmdbSource(final Guid cmdbsource)\n
    '''
def getLabel():
    '''returns String\n\n
    getLabel()\n
    '''
def hasLabel():
    '''returns boolean\n\n
    hasLabel()\n
    '''
def setLabel():
    '''returns None\n\n
    setLabel(final String label)\n
    '''
def getDescription():
    '''returns String\n\n
    getDescription()\n
    '''
def hasDescription():
    '''returns boolean\n\n
    hasDescription()\n
    '''
def setDescription():
    '''returns None\n\n
    setDescription(final String description)\n
    '''
def getAdminState():
    '''returns int\n\n
    getAdminState()\n
    '''
def hasAdminState():
    '''returns boolean\n\n
    hasAdminState()\n
    '''
def setAdminState():
    '''returns None\n\n
    setAdminState(final int state)\n
    '''
def getBidiFlag():
    '''returns int\n\n
    getBidiFlag()\n
    '''
def hasBidiFlag():
    '''returns boolean\n\n
    hasBidiFlag()\n
    '''
def setBidiFlag():
    '''returns None\n\n
    setBidiFlag(final int bidiflag)\n
    '''
def getBidiFormat():
    '''returns String\n\n
    getBidiFormat()\n
    '''
def hasBidiFormat():
    '''returns boolean\n\n
    hasBidiFormat()\n
    '''
def setBidiFormat():
    '''returns None\n\n
    setBidiFormat(final String bidiformat)\n
    '''
def getAllAttributes():
    '''returns Map\n\n
    getAllAttributes()\n
    '''
def getCreatedBy():
    '''returns String\n\n
    getCreatedBy()\n
    '''
def hasCreatedBy():
    '''returns boolean\n\n
    hasCreatedBy()\n
    '''
def getLastModifiedTime():
    '''returns long\n\n
    getLastModifiedTime()\n
    '''
def hasLastModifiedTime():
    '''returns boolean\n\n
    hasLastModifiedTime()\n
    '''
def getLastModifiedBy():
    '''returns String\n\n
    getLastModifiedBy()\n
    '''
def hasLastModifiedBy():
    '''returns boolean\n\n
    hasLastModifiedBy()\n
    '''
def getExtendedAttributes():
    '''returns byte[]\n\n
    getExtendedAttributes()\n
    '''
def hasExtendedAttributes():
    '''returns boolean\n\n
    hasExtendedAttributes()\n
    '''
def setExtendedAttributes():
    '''returns None\n\n
    setExtendedAttributes(final byte[] eAttribMap)\n
    '''
def toString():
    '''returns String\n\n
    toString()\n
    '''
