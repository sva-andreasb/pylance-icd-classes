def ():
    '''returns HAILImpl\n\n
    ()\n
    (final Map m)\n
    '''
def getStateString():
    '''returns String\n\n
    getStateString()\n
    '''
def hasStateString():
    '''returns boolean\n\n
    hasStateString()\n
    '''
def setStateString():
    '''returns None\n\n
    setStateString(final String state)\n
    '''
def getBufferSize():
    '''returns int\n\n
    getBufferSize()\n
    '''
def hasBufferSize():
    '''returns boolean\n\n
    hasBufferSize()\n
    '''
def setBufferSize():
    '''returns None\n\n
    setBufferSize(final int size)\n
    '''
def getChunkSize():
    '''returns int\n\n
    getChunkSize()\n
    '''
def hasChunkSize():
    '''returns boolean\n\n
    hasChunkSize()\n
    '''
def setChunkSize():
    '''returns None\n\n
    setChunkSize(final int size)\n
    '''
def getServerPort():
    '''returns int\n\n
    getServerPort()\n
    '''
def hasServerPort():
    '''returns boolean\n\n
    hasServerPort()\n
    '''
def setServerPort():
    '''returns None\n\n
    setServerPort(final int port)\n
    '''
def getEnableTcpNoDelay():
    '''returns boolean\n\n
    getEnableTcpNoDelay()\n
    '''
def hasEnableTcpNoDelay():
    '''returns boolean\n\n
    hasEnableTcpNoDelay()\n
    '''
def setEnableTcpNoDelay():
    '''returns None\n\n
    setEnableTcpNoDelay(final boolean delay)\n
    '''
def getXAConnectionFactoryJNDIRef():
    '''returns String\n\n
    getXAConnectionFactoryJNDIRef()\n
    '''
def hasXAConnectionFactoryJNDIRef():
    '''returns boolean\n\n
    hasXAConnectionFactoryJNDIRef()\n
    '''
def setXAConnectionFactoryJNDIRef():
    '''returns None\n\n
    setXAConnectionFactoryJNDIRef(final String reference)\n
    '''
def getServerAddress():
    '''returns String\n\n
    getServerAddress()\n
    '''
def hasServerAddress():
    '''returns boolean\n\n
    hasServerAddress()\n
    '''
def setServerAddress():
    '''returns None\n\n
    setServerAddress(final String reference)\n
    '''
def getConnectionFactoryJNDIRef():
    '''returns String\n\n
    getConnectionFactoryJNDIRef()\n
    '''
def hasConnectionFactoryJNDIRef():
    '''returns boolean\n\n
    hasConnectionFactoryJNDIRef()\n
    '''
def setConnectionFactoryJNDIRef():
    '''returns None\n\n
    setConnectionFactoryJNDIRef(final String reference)\n
    '''
def getClientSocketFactory():
    '''returns String\n\n
    getClientSocketFactory()\n
    '''
def hasClientSocketFactory():
    '''returns boolean\n\n
    hasClientSocketFactory()\n
    '''
def setClientSocketFactory():
    '''returns None\n\n
    setClientSocketFactory(final String factory)\n
    '''
def getMasterNode():
    '''returns boolean\n\n
    getMasterNode()\n
    '''
def hasMasterNode():
    '''returns boolean\n\n
    hasMasterNode()\n
    '''
def setMasterNode():
    '''returns None\n\n
    setMasterNode(final boolean node)\n
    '''
def getPingPeriod():
    '''returns long\n\n
    getPingPeriod()\n
    '''
def hasPingPeriod():
    '''returns boolean\n\n
    hasPingPeriod()\n
    '''
def setPingPeriod():
    '''returns None\n\n
    setPingPeriod(final long period)\n
    '''
def getManagedSystemName():
    '''returns String\n\n
    getManagedSystemName()\n
    '''
def hasManagedSystemName():
    '''returns boolean\n\n
    hasManagedSystemName()\n
    '''
def setManagedSystemName():
    '''returns None\n\n
    setManagedSystemName(final String x)\n
    '''
def getRoles():
    '''returns Role[]\n\n
    getRoles()\n
    '''
def hasRoles():
    '''returns boolean\n\n
    hasRoles()\n
    '''
def setRoles():
    '''returns None\n\n
    setRoles(final Role[] x)\n
    '''
def getGuid():
    '''returns Guid\n\n
    getGuid()\n
    '''
def hasGuid():
    '''returns boolean\n\n
    hasGuid()\n
    '''
def setGuid():
    '''returns None\n\n
    setGuid(final Guid guid)\n
    '''
def getDisplayName():
    '''returns String\n\n
    getDisplayName()\n
    '''
def hasDisplayName():
    '''returns boolean\n\n
    hasDisplayName()\n
    '''
def getContextIp():
    '''returns String\n\n
    getContextIp()\n
    '''
def hasContextIp():
    '''returns boolean\n\n
    hasContextIp()\n
    '''
def setContextIp():
    '''returns None\n\n
    setContextIp(final String ctx)\n
    '''
def getObjectType():
    '''returns String\n\n
    getObjectType()\n
    '''
def hasObjectType():
    '''returns boolean\n\n
    hasObjectType()\n
    '''
def setObjectType():
    '''returns None\n\n
    setObjectType(final String type)\n
    '''
def getCDMSource():
    '''returns String\n\n
    getCDMSource()\n
    '''
def hasCDMSource():
    '''returns boolean\n\n
    hasCDMSource()\n
    '''
def setCDMSource():
    '''returns None\n\n
    setCDMSource(final String source)\n
    '''
def getSourceToken():
    '''returns String\n\n
    getSourceToken()\n
    '''
def hasSourceToken():
    '''returns boolean\n\n
    hasSourceToken()\n
    '''
def setSourceToken():
    '''returns None\n\n
    setSourceToken(final String sourceToken)\n
    '''
def getCmdbSource():
    '''returns Guid\n\n
    getCmdbSource()\n
    '''
def hasCmdbSource():
    '''returns boolean\n\n
    hasCmdbSource()\n
    '''
def setCmdbSource():
    '''returns None\n\n
    setCmdbSource(final Guid cmdbsource)\n
    '''
def getLabel():
    '''returns String\n\n
    getLabel()\n
    '''
def hasLabel():
    '''returns boolean\n\n
    hasLabel()\n
    '''
def setLabel():
    '''returns None\n\n
    setLabel(final String label)\n
    '''
def getDescription():
    '''returns String\n\n
    getDescription()\n
    '''
def hasDescription():
    '''returns boolean\n\n
    hasDescription()\n
    '''
def setDescription():
    '''returns None\n\n
    setDescription(final String description)\n
    '''
def getAdminState():
    '''returns int\n\n
    getAdminState()\n
    '''
def hasAdminState():
    '''returns boolean\n\n
    hasAdminState()\n
    '''
def setAdminState():
    '''returns None\n\n
    setAdminState(final int state)\n
    '''
def getBidiFlag():
    '''returns int\n\n
    getBidiFlag()\n
    '''
def hasBidiFlag():
    '''returns boolean\n\n
    hasBidiFlag()\n
    '''
def setBidiFlag():
    '''returns None\n\n
    setBidiFlag(final int bidiflag)\n
    '''
def getBidiFormat():
    '''returns String\n\n
    getBidiFormat()\n
    '''
def hasBidiFormat():
    '''returns boolean\n\n
    hasBidiFormat()\n
    '''
def setBidiFormat():
    '''returns None\n\n
    setBidiFormat(final String bidiformat)\n
    '''
def getAllAttributes():
    '''returns Map\n\n
    getAllAttributes()\n
    '''
def getCreatedBy():
    '''returns String\n\n
    getCreatedBy()\n
    '''
def hasCreatedBy():
    '''returns boolean\n\n
    hasCreatedBy()\n
    '''
def getLastModifiedTime():
    '''returns long\n\n
    getLastModifiedTime()\n
    '''
def hasLastModifiedTime():
    '''returns boolean\n\n
    hasLastModifiedTime()\n
    '''
def getLastModifiedBy():
    '''returns String\n\n
    getLastModifiedBy()\n
    '''
def hasLastModifiedBy():
    '''returns boolean\n\n
    hasLastModifiedBy()\n
    '''
def getExtendedAttributes():
    '''returns byte[]\n\n
    getExtendedAttributes()\n
    '''
def hasExtendedAttributes():
    '''returns boolean\n\n
    hasExtendedAttributes()\n
    '''
def setExtendedAttributes():
    '''returns None\n\n
    setExtendedAttributes(final byte[] eAttribMap)\n
    '''
def getName():
    '''returns String\n\n
    getName()\n
    '''
def hasName():
    '''returns boolean\n\n
    hasName()\n
    '''
def setName():
    '''returns None\n\n
    setName(final String name)\n
    '''
def getParent():
    '''returns J2EEDomain\n\n
    getParent()\n
    '''
def hasParent():
    '''returns boolean\n\n
    hasParent()\n
    '''
def setParent():
    '''returns None\n\n
    setParent(final J2EEDomain jmsServer)\n
    '''
def toString():
    '''returns String\n\n
    toString()\n
    '''
