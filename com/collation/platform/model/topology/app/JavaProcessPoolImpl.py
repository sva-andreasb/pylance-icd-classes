def ():
    '''returns JavaProcessPoolImpl\n\n
    ()\n
    (final Map m)\n
    '''
def getManagedSystemName():
    '''returns String\n\n
    getManagedSystemName()\n
    '''
def hasManagedSystemName():
    '''returns boolean\n\n
    hasManagedSystemName()\n
    '''
def setManagedSystemName():
    '''returns None\n\n
    setManagedSystemName(final String x)\n
    '''
def getRoles():
    '''returns Role[]\n\n
    getRoles()\n
    '''
def hasRoles():
    '''returns boolean\n\n
    hasRoles()\n
    '''
def setRoles():
    '''returns None\n\n
    setRoles(final Role[] x)\n
    '''
def getGuid():
    '''returns Guid\n\n
    getGuid()\n
    '''
def hasGuid():
    '''returns boolean\n\n
    hasGuid()\n
    '''
def setGuid():
    '''returns None\n\n
    setGuid(final Guid guid)\n
    '''
def getDisplayName():
    '''returns String\n\n
    getDisplayName()\n
    '''
def hasDisplayName():
    '''returns boolean\n\n
    hasDisplayName()\n
    '''
def getContextIp():
    '''returns String\n\n
    getContextIp()\n
    '''
def hasContextIp():
    '''returns boolean\n\n
    hasContextIp()\n
    '''
def setContextIp():
    '''returns None\n\n
    setContextIp(final String ctx)\n
    '''
def getObjectType():
    '''returns String\n\n
    getObjectType()\n
    '''
def hasObjectType():
    '''returns boolean\n\n
    hasObjectType()\n
    '''
def setObjectType():
    '''returns None\n\n
    setObjectType(final String type)\n
    '''
def getCDMSource():
    '''returns String\n\n
    getCDMSource()\n
    '''
def hasCDMSource():
    '''returns boolean\n\n
    hasCDMSource()\n
    '''
def setCDMSource():
    '''returns None\n\n
    setCDMSource(final String source)\n
    '''
def getSourceToken():
    '''returns String\n\n
    getSourceToken()\n
    '''
def hasSourceToken():
    '''returns boolean\n\n
    hasSourceToken()\n
    '''
def setSourceToken():
    '''returns None\n\n
    setSourceToken(final String sourceToken)\n
    '''
def getCmdbSource():
    '''returns Guid\n\n
    getCmdbSource()\n
    '''
def hasCmdbSource():
    '''returns boolean\n\n
    hasCmdbSource()\n
    '''
def setCmdbSource():
    '''returns None\n\n
    setCmdbSource(final Guid cmdbsource)\n
    '''
def getLabel():
    '''returns String\n\n
    getLabel()\n
    '''
def hasLabel():
    '''returns boolean\n\n
    hasLabel()\n
    '''
def setLabel():
    '''returns None\n\n
    setLabel(final String label)\n
    '''
def getDescription():
    '''returns String\n\n
    getDescription()\n
    '''
def hasDescription():
    '''returns boolean\n\n
    hasDescription()\n
    '''
def setDescription():
    '''returns None\n\n
    setDescription(final String description)\n
    '''
def getAdminState():
    '''returns int\n\n
    getAdminState()\n
    '''
def hasAdminState():
    '''returns boolean\n\n
    hasAdminState()\n
    '''
def setAdminState():
    '''returns None\n\n
    setAdminState(final int state)\n
    '''
def getBidiFlag():
    '''returns int\n\n
    getBidiFlag()\n
    '''
def hasBidiFlag():
    '''returns boolean\n\n
    hasBidiFlag()\n
    '''
def setBidiFlag():
    '''returns None\n\n
    setBidiFlag(final int bidiflag)\n
    '''
def getBidiFormat():
    '''returns String\n\n
    getBidiFormat()\n
    '''
def hasBidiFormat():
    '''returns boolean\n\n
    hasBidiFormat()\n
    '''
def setBidiFormat():
    '''returns None\n\n
    setBidiFormat(final String bidiformat)\n
    '''
def getAllAttributes():
    '''returns Map\n\n
    getAllAttributes()\n
    '''
def getCreatedBy():
    '''returns String\n\n
    getCreatedBy()\n
    '''
def hasCreatedBy():
    '''returns boolean\n\n
    hasCreatedBy()\n
    '''
def getLastModifiedTime():
    '''returns long\n\n
    getLastModifiedTime()\n
    '''
def hasLastModifiedTime():
    '''returns boolean\n\n
    hasLastModifiedTime()\n
    '''
def getLastModifiedBy():
    '''returns String\n\n
    getLastModifiedBy()\n
    '''
def hasLastModifiedBy():
    '''returns boolean\n\n
    hasLastModifiedBy()\n
    '''
def getExtendedAttributes():
    '''returns byte[]\n\n
    getExtendedAttributes()\n
    '''
def hasExtendedAttributes():
    '''returns boolean\n\n
    hasExtendedAttributes()\n
    '''
def setExtendedAttributes():
    '''returns None\n\n
    setExtendedAttributes(final byte[] eAttribMap)\n
    '''
def getParent():
    '''returns AppServer\n\n
    getParent()\n
    '''
def hasParent():
    '''returns boolean\n\n
    hasParent()\n
    '''
def setParent():
    '''returns None\n\n
    setParent(final AppServer parent)\n
    '''
def getName():
    '''returns String\n\n
    getName()\n
    '''
def hasName():
    '''returns boolean\n\n
    hasName()\n
    '''
def setName():
    '''returns None\n\n
    setName(final String name)\n
    '''
def getExecutable():
    '''returns DataFile\n\n
    getExecutable()\n
    '''
def hasExecutable():
    '''returns boolean\n\n
    hasExecutable()\n
    '''
def setExecutable():
    '''returns None\n\n
    setExecutable(final DataFile file)\n
    '''
def getCmdLine():
    '''returns String\n\n
    getCmdLine()\n
    '''
def hasCmdLine():
    '''returns boolean\n\n
    hasCmdLine()\n
    '''
def setCmdLine():
    '''returns None\n\n
    setCmdLine(final String cmdLine)\n
    '''
def getCWD():
    '''returns String\n\n
    getCWD()\n
    '''
def hasCWD():
    '''returns boolean\n\n
    hasCWD()\n
    '''
def setCWD():
    '''returns None\n\n
    setCWD(final String cwd)\n
    '''
def getEnv():
    '''returns String\n\n
    getEnv()\n
    '''
def hasEnv():
    '''returns boolean\n\n
    hasEnv()\n
    '''
def setEnv():
    '''returns None\n\n
    setEnv(final String env)\n
    '''
def getWindowsServiceList():
    '''returns String\n\n
    getWindowsServiceList()\n
    '''
def hasWindowsServiceList():
    '''returns boolean\n\n
    hasWindowsServiceList()\n
    '''
def setWindowsServiceList():
    '''returns None\n\n
    setWindowsServiceList(final String windowsServiceList)\n
    '''
def getInitialSize():
    '''returns int\n\n
    getInitialSize()\n
    '''
def hasInitialSize():
    '''returns boolean\n\n
    hasInitialSize()\n
    '''
def setInitialSize():
    '''returns None\n\n
    setInitialSize(final int initialSize)\n
    '''
def getMaxSize():
    '''returns int\n\n
    getMaxSize()\n
    '''
def hasMaxSize():
    '''returns boolean\n\n
    hasMaxSize()\n
    '''
def setMaxSize():
    '''returns None\n\n
    setMaxSize(final int maxSize)\n
    '''
def getMinSize():
    '''returns int\n\n
    getMinSize()\n
    '''
def hasMinSize():
    '''returns boolean\n\n
    hasMinSize()\n
    '''
def setMinSize():
    '''returns None\n\n
    setMinSize(final int minSize)\n
    '''
def getIncrement():
    '''returns int\n\n
    getIncrement()\n
    '''
def hasIncrement():
    '''returns boolean\n\n
    hasIncrement()\n
    '''
def setIncrement():
    '''returns None\n\n
    setIncrement(final int increment)\n
    '''
def getDecrement():
    '''returns int\n\n
    getDecrement()\n
    '''
def hasDecrement():
    '''returns boolean\n\n
    hasDecrement()\n
    '''
def setDecrement():
    '''returns None\n\n
    setDecrement(final int decrement)\n
    '''
def getSize():
    '''returns int\n\n
    getSize()\n
    '''
def hasSize():
    '''returns boolean\n\n
    hasSize()\n
    '''
def setSize():
    '''returns None\n\n
    setSize(final int size)\n
    '''
def getRuntimeProcesses():
    '''returns RuntimeProcess[]\n\n
    getRuntimeProcesses()\n
    '''
def hasRuntimeProcesses():
    '''returns boolean\n\n
    hasRuntimeProcesses()\n
    '''
def setRuntimeProcesses():
    '''returns None\n\n
    setRuntimeProcesses(final RuntimeProcess[] processes)\n
    '''
def toString():
    '''returns String\n\n
    toString()\n
    '''
