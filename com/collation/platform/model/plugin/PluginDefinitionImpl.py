def PluginDefinitionImpl():
'''public PluginDefinitionImpl()
public PluginDefinitionImpl(final Map m)
'''
pass
def getPluginType():
'''public String getPluginType()
'''
pass
def hasPluginType():
'''public boolean hasPluginType()
'''
pass
def setPluginType():
'''public void setPluginType(final String pluginType)
'''
pass
def getCommandName():
'''public String getCommandName()
'''
pass
def hasCommandName():
'''public boolean hasCommandName()
'''
pass
def setCommandName():
'''public void setCommandName(final String commandName)
'''
pass
def getLocalWorkingDirectory():
'''public String getLocalWorkingDirectory()
'''
pass
def hasLocalWorkingDirectory():
'''public boolean hasLocalWorkingDirectory()
'''
pass
def setLocalWorkingDirectory():
'''public void setLocalWorkingDirectory(final String localLocation)
'''
pass
def getCommandRemote():
'''public boolean getCommandRemote()
'''
pass
def hasCommandRemote():
'''public boolean hasCommandRemote()
'''
pass
def setCommandRemote():
'''public void setCommandRemote(final boolean runRemotely)
'''
pass
def getRemoteWorkingDirectory():
'''public String getRemoteWorkingDirectory()
'''
pass
def hasRemoteWorkingDirectory():
'''public boolean hasRemoteWorkingDirectory()
'''
pass
def setRemoteWorkingDirectory():
'''public void setRemoteWorkingDirectory(final String remoteLocation)
'''
pass
def getGuid():
'''public Guid getGuid()
'''
pass
def hasGuid():
'''public boolean hasGuid()
'''
pass
def setGuid():
'''public void setGuid(final Guid guid)
'''
pass
def getDisplayName():
'''public String getDisplayName()
'''
pass
def hasDisplayName():
'''public boolean hasDisplayName()
'''
pass
def getContextIp():
'''public String getContextIp()
'''
pass
def hasContextIp():
'''public boolean hasContextIp()
'''
pass
def setContextIp():
'''public void setContextIp(final String ctx)
'''
pass
def getObjectType():
'''public String getObjectType()
'''
pass
def hasObjectType():
'''public boolean hasObjectType()
'''
pass
def setObjectType():
'''public void setObjectType(final String type)
'''
pass
def getCDMSource():
'''public String getCDMSource()
'''
pass
def hasCDMSource():
'''public boolean hasCDMSource()
'''
pass
def setCDMSource():
'''public void setCDMSource(final String source)
'''
pass
def getSourceToken():
'''public String getSourceToken()
'''
pass
def hasSourceToken():
'''public boolean hasSourceToken()
'''
pass
def setSourceToken():
'''public void setSourceToken(final String sourceToken)
'''
pass
def getCmdbSource():
'''public Guid getCmdbSource()
'''
pass
def hasCmdbSource():
'''public boolean hasCmdbSource()
'''
pass
def setCmdbSource():
'''public void setCmdbSource(final Guid cmdbsource)
'''
pass
def getLabel():
'''public String getLabel()
'''
pass
def hasLabel():
'''public boolean hasLabel()
'''
pass
def setLabel():
'''public void setLabel(final String label)
'''
pass
def getDescription():
'''public String getDescription()
'''
pass
def hasDescription():
'''public boolean hasDescription()
'''
pass
def setDescription():
'''public void setDescription(final String description)
'''
pass
def getAdminState():
'''public int getAdminState()
'''
pass
def hasAdminState():
'''public boolean hasAdminState()
'''
pass
def setAdminState():
'''public void setAdminState(final int state)
'''
pass
def getBidiFlag():
'''public int getBidiFlag()
'''
pass
def hasBidiFlag():
'''public boolean hasBidiFlag()
'''
pass
def setBidiFlag():
'''public void setBidiFlag(final int bidiflag)
'''
pass
def getBidiFormat():
'''public String getBidiFormat()
'''
pass
def hasBidiFormat():
'''public boolean hasBidiFormat()
'''
pass
def setBidiFormat():
'''public void setBidiFormat(final String bidiformat)
'''
pass
def getAllAttributes():
'''public Map getAllAttributes()
'''
pass
def getCreatedBy():
'''public String getCreatedBy()
'''
pass
def hasCreatedBy():
'''public boolean hasCreatedBy()
'''
pass
def getLastModifiedTime():
'''public long getLastModifiedTime()
'''
pass
def hasLastModifiedTime():
'''public boolean hasLastModifiedTime()
'''
pass
def getLastModifiedBy():
'''public String getLastModifiedBy()
'''
pass
def hasLastModifiedBy():
'''public boolean hasLastModifiedBy()
'''
pass
def getExtendedAttributes():
'''public byte[] getExtendedAttributes()
'''
pass
def hasExtendedAttributes():
'''public boolean hasExtendedAttributes()
'''
pass
def setExtendedAttributes():
'''public void setExtendedAttributes(final byte[] eAttribMap)
'''
pass
def getName():
'''public String getName()
'''
pass
def hasName():
'''public boolean hasName()
'''
pass
def setName():
'''public void setName(final String name)
'''
pass
def getFamilyName():
'''public String getFamilyName()
'''
pass
def hasFamilyName():
'''public boolean hasFamilyName()
'''
pass
def setFamilyName():
'''public void setFamilyName(final String name)
'''
pass
def getEnabled():
'''public boolean getEnabled()
'''
pass
def hasEnabled():
'''public boolean hasEnabled()
'''
pass
def setEnabled():
'''public void setEnabled(final boolean enabled)
'''
pass
def getOperationalStatus():
'''public int getOperationalStatus()
'''
pass
def hasOperationalStatus():
'''public boolean hasOperationalStatus()
'''
pass
def setOperationalStatus():
'''public void setOperationalStatus(final int status)
'''
pass
def getRejectionReason():
'''public String getRejectionReason()
'''
pass
def hasRejectionReason():
'''public boolean hasRejectionReason()
'''
pass
def setRejectionReason():
'''public void setRejectionReason(final String reason)
'''
pass
def getMssGuid():
'''public String getMssGuid()
'''
pass
def hasMssGuid():
'''public boolean hasMssGuid()
'''
pass
def setMssGuid():
'''public void setMssGuid(final String mssGuid)
'''
pass
def toString():
'''public String toString()
'''
pass
