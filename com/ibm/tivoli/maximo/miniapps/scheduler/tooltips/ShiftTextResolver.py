def ():
    '''returns ShiftTextResolver\n\n
    (final AbstractTreeGridMiniAppBean.ShiftInfo shiftInfo, final AbstractTreeGridMiniAppBean.ShiftInfo breakInfo, final IMXActivity modAvail, final IMXGanttModel model, final UserInfo info)\n
    '''
def getProperty():
    '''returns Object\n\n
    getProperty(final Map<String, Object> data, final String in)\n
    '''
def getColumnInfo():
    '''returns IMXGanttPropertyInfo\n\n
    getColumnInfo(final String in)\n
    '''
def getPropertyNames():
    '''returns Collection<String>\n\n
    getPropertyNames(final Map<String, Object> data)\n
    '''
