def ():
    '''returns IlvActivityCSSModel\n\n
    (final IlvHierarchyChart ilvHierarchyChart, final IlvBeanStateCache ilvBeanStateCache)\n
    '''
def propertyChange():
    '''returns None\n\n
    propertyChange(final PropertyChangeEvent propertyChangeEvent)\n
    '''
def getGanttModel():
    '''returns IlvGanttModel\n\n
    getGanttModel()\n
    '''
def isStylingEnabled():
    '''returns boolean\n\n
    isStylingEnabled()\n
    '''
def getCSSClassesProperty():
    '''returns String\n\n
    getCSSClassesProperty()\n
    '''
def getNodePredicate():
    '''returns IlvUnaryPredicate\n\n
    getNodePredicate()\n
    '''
def evaluate():
    '''returns boolean\n\n
    evaluate(final Object o)\n
    '''
def getRoot():
    '''returns Object\n\n
    getRoot()\n
    '''
def getID():
    '''returns String\n\n
    getID(final Object o)\n
    '''
def getType():
    '''returns String\n\n
    getType(final Object o)\n
    '''
def getCSSclasses():
    '''returns String\n\n
    getCSSclasses(final Object o)\n
    '''
def getCSSPseudoClasses():
    '''returns String[]\n\n
    getCSSPseudoClasses(final Object o)\n
    '''
def getValueAsObject():
    '''returns Object\n\n
    getValueAsObject(final Object o, final String s)\n
    '''
def getChildrenAsArray():
    '''returns Object[]\n\n
    getChildrenAsArray(final Object o)\n
    '''
def getULocale():
    '''returns ULocale\n\n
    getULocale(final Object o)\n
    '''
def getComponentOrientation():
    '''returns ComponentOrientation\n\n
    getComponentOrientation(final Object o)\n
    '''
def getBean():
    '''returns Object\n\n
    getBean(final Object o)\n
    '''
def setBeansInterpreter():
    '''returns None\n\n
    setBeansInterpreter(final IlvBeansInterpretation beansInterpreter)\n
    '''
def customize():
    '''returns None\n\n
    customize()\n
    '''
def customizeTree():
    '''returns None\n\n
    customizeTree(final Object o)\n
    '''
def setChart():
    '''returns None\n\n
    setChart(final IlvHierarchyChart a)\n
    setChart(final IlvHierarchyChart a)\n
    '''
def selectionChanged():
    '''returns None\n\n
    selectionChanged(final SelectionEvent selectionEvent)\n
    '''
def ganttModelChanged():
    '''returns None\n\n
    ganttModelChanged(final GanttModelChangedEvent ganttModelChangedEvent)\n
    '''
def activityChanged():
    '''returns None\n\n
    activityChanged(final ActivityEvent activityEvent)\n
    '''
def createActivityRenderer():
    '''returns IlvActivityRenderer\n\n
    createActivityRenderer(final IlvActivity ilvActivity)\n
    '''
