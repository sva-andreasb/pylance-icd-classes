def ():
    '''returns IlvFacesDHTMLDashboardView\n\n
    ()\n
    '''
def getFamily():
    '''returns String\n\n
    getFamily()\n
    '''
def getBackgroundColor():
    '''returns Color\n\n
    getBackgroundColor()\n
    '''
def getControllerURL():
    '''returns String\n\n
    getControllerURL()\n
    '''
def getErrorMessage():
    '''returns String\n\n
    getErrorMessage()\n
    '''
def getImageFormat():
    '''returns String\n\n
    getImageFormat()\n
    '''
def getOnCapabilitiesLoaded():
    '''returns String\n\n
    getOnCapabilitiesLoaded()\n
    '''
def getOnImageLoaded():
    '''returns String\n\n
    getOnImageLoaded()\n
    '''
def getServlet():
    '''returns String\n\n
    getServlet()\n
    '''
def getServletClass():
    '''returns String\n\n
    getServletClass()\n
    '''
def getServletURL():
    '''returns String\n\n
    getServletURL()\n
    '''
def getStateURL():
    '''returns String\n\n
    getStateURL()\n
    '''
def getUpdateInterval():
    '''returns int\n\n
    getUpdateInterval()\n
    '''
def getWaitingImage():
    '''returns String\n\n
    getWaitingImage()\n
    '''
def isGenerateImageMap():
    '''returns boolean\n\n
    isGenerateImageMap()\n
    '''
def isImageMapVisible():
    '''returns boolean\n\n
    isImageMapVisible()\n
    '''
def setBackgroundColor():
    '''returns None\n\n
    setBackgroundColor(final Color backgroundColor)\n
    '''
def setErrorMessage():
    '''returns None\n\n
    setErrorMessage(final String errorMessage)\n
    '''
def setGenerateImageMap():
    '''returns None\n\n
    setGenerateImageMap(final boolean generateImageMap)\n
    '''
def setImageFormat():
    '''returns None\n\n
    setImageFormat(final String imageFormat)\n
    '''
def setImageMapVisible():
    '''returns None\n\n
    setImageMapVisible(final boolean imageMapVisible)\n
    '''
def getImageMapGenerator():
    '''returns IlvImageMapAreaGenerator\n\n
    getImageMapGenerator()\n
    '''
def setImageMapGenerator():
    '''returns None\n\n
    setImageMapGenerator(final IlvImageMapAreaGenerator imageMapGenerator)\n
    '''
def getImageMapGeneratorClass():
    '''returns String\n\n
    getImageMapGeneratorClass()\n
    '''
def setImageMapGeneratorClass():
    '''returns None\n\n
    setImageMapGeneratorClass(final String imageMapGeneratorClass)\n
    '''
def setOnCapabilitiesLoaded():
    '''returns None\n\n
    setOnCapabilitiesLoaded(final String onCapabilitiesLoaded)\n
    '''
def setOnImageLoaded():
    '''returns None\n\n
    setOnImageLoaded(final String onImageLoaded)\n
    '''
def setServlet():
    '''returns None\n\n
    setServlet(final String value)\n
    '''
def setServletClass():
    '''returns None\n\n
    setServletClass(final String servletClass)\n
    '''
def setServletURL():
    '''returns None\n\n
    setServletURL(final String servletURL)\n
    '''
def setStateURL():
    '''returns None\n\n
    setStateURL(final String stateURL)\n
    '''
def setUpdateInterval():
    '''returns None\n\n
    setUpdateInterval(final int updateInterval)\n
    '''
def setValue():
    '''returns None\n\n
    setValue(final Object o)\n
    '''
def setWaitingImage():
    '''returns None\n\n
    setWaitingImage(final String waitingImage)\n
    '''
def isResizable():
    '''returns boolean\n\n
    isResizable()\n
    '''
def setResizable():
    '''returns None\n\n
    setResizable(final boolean resizable)\n
    '''
def isAccessible():
    '''returns boolean\n\n
    isAccessible()\n
    '''
def setAccessible():
    '''returns None\n\n
    setAccessible(final boolean accessible)\n
    '''
def getTileManager():
    '''returns IlvTileManager\n\n
    getTileManager()\n
    '''
def setTileManager():
    '''returns None\n\n
    setTileManager(final IlvTileManager tileManager)\n
    '''
def getBeforeSessionExpirationHandler():
    '''returns String\n\n
    getBeforeSessionExpirationHandler()\n
    '''
def setBeforeSessionExpirationHandler():
    '''returns None\n\n
    setBeforeSessionExpirationHandler(final String beforeSessionExpirationHandler)\n
    '''
def getView():
    '''returns IlvManagerView\n\n
    getView()\n
    '''
def getDashboardDiagram():
    '''returns IlvDashboardDiagram\n\n
    getDashboardDiagram()\n
    '''
def updateModel():
    '''returns None\n\n
    updateModel(final FacesContext facesContext)\n
    '''
def saveState():
    '''returns Object\n\n
    saveState(final FacesContext facesContext)\n
    '''
def restoreState():
    '''returns None\n\n
    restoreState(final FacesContext facesContext, final Object o)\n
    '''
